...
% % example 1, 
% Results =StokesDispSolver('h',1,'T', 5, 'a',0.15, 'mode',1)
% % mode 2, the first harmonic term's coefficient is given as aw, Hw
% % example 2, 
% Results =StokesDispSolver('h',1,'T', 5, 'a',0.15, 'mode',2)
% % example 3, 
% Results =StokesDispSolver('h',1,'T', 5, 'H',0.3, 'mode',1)
% % example 4, 
% Results =StokesDispSolver('h',1,'T', 5, 'H',0.3, 'mode',2)
% % example 5, 
% Results =StokesDispSolver('h',1,'omega0', 1.2566, 'H',0.3, 'mode',2)
...
%     [B31,B33,B51,B53,B55] = findBcoeff(h,kResults);
    switch modeNo
        case 1
            ...
            Results.H = Hout;
        case 2
            Hout = 2*aw+2*B33*kResults^2*aw^3+2*(B53-3*B31+B55)*kResults^4*aw^5;
            Results.Hw = Hout;
    end
...
%         [B31,B33,B51,B53,B55] = findBcoeff(h,k);
    switch modeNo
        ...
        case 2
            Hw = H;
            Results.Hw = Hw;
            Hfun2 = Hw - (2*aw+2*B33*k^2*aw^3+2*(B53-3*B31+B55)*k^4*aw^5)==0;
            omega2 = k^2*aw^2*(2*alpha1^2+7)/4/(alpha1 -1)^2;
            omega4 = (aw^4*k^4*(18*alpha1^6 + 108*alpha1^5 + 29*alpha1^4 ...
                - 244*alpha1^3 - 201*alpha1^2 + 73*alpha1 + 379))/(32*(alpha1 - 1)^5*(alpha1 + 1));
...
%         [B31,B33,B51,B53,B55] = findBcoeff(h,k);
    switch modeNo
        ...
        case 2
            Hw = H;
            Results.Hw = Hw;
            Hfun2 = Hw - (2*aw+2*B33*kResults^2*aw^3+2*(B53-3*B31+B55)*kResults^4*aw^5)==0;
