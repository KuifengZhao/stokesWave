...
if and(exist('a', 'var') ==1, exist('T', 'var') ==1)
    ...
    if modeNo == 1 
        ...
        B31 = (3+8*sigma^2-9*sigma^4)/16/sigma^4;%(cosh(2*h*kResults)^2 + 12*cosh(2*h*kResults) - 7)/(8*(cosh(2*h*kResults) - 1)^2);
    ...
    elseif modeNo == 2 % in case first harmonic wave amplitude aw is given
        ...
        omega4 = (a0^4*k^4*(20*alpha1^6 + 132*alpha1^5 + 20*alpha1^4 ...
        - 184*alpha1^3 - 243*alpha1^2 - 11*alpha1 + 428))/(32*(alpha1 - 1)^5*(alpha1 + 1));
        omegaFun = omega0*(1+omega2+omega4) - omega; 
    ...
    sigma = tanh(kResults*h);
    B31 = (3+8*sigma^2-9*sigma^4)/16/sigma^4;%(cosh(2*h*kResults)^2 + 12*cosh(2*h*kResults) - 7)/(8*(cosh(2*h*kResults) - 1)^2);
    B33 = (27-9*sigma^2+9*sigma^4-3*sigma^6)/64/sigma^6; %(cosh(2*h*kResults)^3 + 3*cosh(2*h*kResults)^2 + 3*cosh(2*h*kResults) + 2)/(8*(cosh(2*h*kResults) - 1)^3);  
    B51 = (121*cosh(2*h*kResults)^6 + 432*cosh(2*h*kResults)^5 + 543*cosh(2*h*kResults)^4 - 1407*cosh(2*h*kResults)^3 ...
    - 258*cosh(2*h*kResults)^2 + 2001*cosh(2*h*kResults) - 1108)/(192*(cosh(2*h*kResults) - 1)^5*(cosh(2*h*kResults) + 1));
...
elseif and(exist('H', 'var') ==1, exist('T', 'var') ==1)
    ...
    syms a
     sigma = tanh(k*h);
        B33 = (27-9*sigma^2+9*sigma^4-3*sigma^6)/64/sigma^6; %(cosh(2*h*k)^3 + 3*cosh(2*h*k)^2 + 3*cosh(2*h*k) + 2)/(8*(cosh(2*h*k) - 1)^3);        
        ...
        B31 = (3+8*sigma^2-9*sigma^4)/16/sigma^4;%(cosh(2*h*k)^2 + 12*cosh(2*h*k) - 7)/(8*(cosh(2*h*k) - 1)^2);
        B51 = (121*cosh(2*h*k)^6 + 432*cosh(2*h*k)^5 + 543*cosh(2*h*k)^4 - 1407*cosh(2*h*k)^3 ...
            - 258*cosh(2*h*k)^2 + 2001*cosh(2*h*k) - 1108)/(192*(cosh(2*h*k) - 1)^5*(cosh(2*h*k) + 1));
...
elseif and(exist('H', 'var') ==1, exist('omega0', 'var') ==1)
    ...
    omega4 = (aResults^4*kResults^4*(20*alpha1^6 + 132*alpha1^5 + 20*alpha1^4 ...
        - 184*alpha1^3 - 243*alpha1^2 - 11*alpha1 + 428))/(32*(alpha1 - 1)^5*(alpha1 + 1));
    omega = omega0*(1+omega2+omega4);
    Results.a = aResults;
    Results.omega = omega;
